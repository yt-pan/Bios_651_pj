...
pred_death_blc <- pred_death_blc %>%
          ...
          unique() %>%
          ungroup(center) %>%
          mutate(uci = obs/(pred+1.96*se)) %>%
          mutate(lci = obs/(pred-1.96*se)) %>%
          mutate(sdr = obs/pred) 
...
plot_death_blc <- pred_death_blc %>% 
  ...
  annotate("text", x=10, y="center 19", label="Reference=1", size=5) +
...
plot_fail_blc <- pred_fail_blc %>% 
  ...
  annotate("text", x=2, y="center 4", label="Reference=1", size=5) +
